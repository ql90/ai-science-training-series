...
print()
print(f'hvd.size: {hvd.size()}, hvd.rank: {hvd.rank()}, hvd.local rank: {hvd.local_rank()}')
print()
...
for gpu in gpus:
        tf.config.experimental.set_memory_growth(gpu, True)
if gpus:
    # gpu_index = hvd.local_rank()
    gpu_index = hvd.local_rank() % len(gpus)
...
print()
print(f'number of samples: {nsamples}, number of tests: {ntests}')
print()

nstep = nsamples//args.batch_size//hvd.size()
nstep = nstep if nstep >= 1 else 1
ntest_step = ntests//args.batch_size//hvd.size()  
ntest_step = ntest_step if ntest_step >= 1 else 1
print(f'number of sample step: {nstep}, number of test step: {ntest_step}')
...
opt = tf.keras.optimizers.Adam(learning_rate=0.00005*hvd.size())

checkpoint_dir = './checkpoints_'+ str(hvd.size())+'/tf2_mnist' 
...
for ep in range(args.epochs):
    ...
    for batch, (images, labels) in enumerate(dataset.take(nstep)):
        training_loss, training_acc = training_step(images, labels)
        ...
        if (hvd.rank() == 0 and batch % 100 == 0): 
            checkpoint.save(checkpoint_dir)
            print('Epoch - %d, step #%06d/%06d\tLoss: %.6f' % (ep, batch, nstep, training_loss))

    # Testing
    test_acc = 0.0
    test_loss = 0.0
    for batch, (images, labels) in enumerate(test_dset.take(ntest_step)):
        test_loss, test_acc = validation_step(images, labels)
...
# checkpoint.save(checkpoint_dir)
metrics_dir = 'metrics_' + str(hvd.size()) + '.dat'
np.savetxt(metrics_dir, np.array([metrics['train_acc'], metrics['train_loss'], metrics['valid_acc'], metrics['valid_loss'], metrics['time_per_epochs']]).transpose())
t1 = time.time()
